...
datatype fs_entry = File of int
                  | Dir of (string * fs_entry) list

fun totalSize (File size) = size
  | totalSize (Dir entries) = map (totalSize o #2) entries |> sum
  ...
  | readLsResult acc (l :: ls) =
    if String.isPrefix "$" l then
        ...
        case words l of
          ...
          | [size, name] => readLsResult ((name, File (valOf (Int.fromString size))) :: acc) ls
          | _ => raise Fail ("Couldn't parse ls result: " ^ l)

fun toTree ([] : string list) : (string * fs_entry) list * string list = ([], [])
  | toTree (_ :: rest) =
    let val (files, rest) = readLsResult [] rest
...
fun allSizes total below [] = total :: below
  | allSizes total below ((_, File s) :: rest) = allSizes (total + s) below rest
  | allSizes total below ((_, Dir d) :: rest) =
    case allSizes 0 [] d of
        (this :: below') => allSizes (total + this) (this :: below' @ below) rest
...
fun solve1 input =
    lines input
          |> rootTree
          |> allSizes 0 []
          |> List.filter (curry op>= 100000)
          |> sum

fun solve2 input : int =
    let val sizes as (total :: _) =
            lines input
                  |> rootTree
                  |> allSizes 0 []
    in sizes
           |> Listsort.sort Int.compare
           |> List.filter (fn x => total - x <= 40000000)
           |> hd
    end
;
